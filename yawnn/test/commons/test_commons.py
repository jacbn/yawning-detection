...
class TestCommons(unittest.TestCase):
    def test_directoryToModelData(self):
        commons.YAWN_TIME = 2
        (x1, y1), (x2, y2) = commons.directoryToModelData(f"{commons.PROJECT_ROOT}/test/test_data/directory_test", eimuLSTM.EimuLSTMInput(sessionGap=1), equalPositiveAndNegative=False)
    ...
    def test_equalizePositiveAndNegative(self):
        (_, y1), (_, y2) = commons.directoryToModelData(f"{commons.PROJECT_ROOT}/test/test_data/directory_test", eimuLSTM.EimuLSTMInput())
