...
class TestEimuModelInput(unittest.TestCase):
    def test_fromPathOnRegularData(self):
        ...
        data, annotations = spectrogramModelInput.SpectrogramModelInput(dataFilter=filters.NormalisationFilter(), windowSize=YAWN_TIME, windowSep=1/32, nPerSeg=128, nOverlap=96).fromPath(f"{PROJECT_ROOT}/test/test_data/high_freq.eimu")
    ...
    def test_fromPathOnShortData(self):
        ...
        with self.assertRaises(ValueError):
            _ = spectrogramModelInput.SpectrogramModelInput(dataFilter=filters.NormalisationFilter(), windowSize=YAWN_TIME, windowSep=1/32).fromPath(f"{PROJECT_ROOT}/test/test_data/short1.eimu")

    def test_fromPathOnRegularDataWithFilter(self):
        YAWN_TIME = config.get("YAWN_TIME")
        # check that none of the following throw an exception
        spectrogramModelInput.SpectrogramModelInput(windowSize=YAWN_TIME, windowSep=3/32, dataFilter=filters.FilterCollection([filters.NormalisationFilter(), filters.MovingAverageFilter(5)])).fromPath(f"{PROJECT_ROOT}/test/test_data/high_freq.eimu")
