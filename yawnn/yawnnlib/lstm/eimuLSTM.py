...
def eimuToLSTMInput(eimuPath : str, dataFilter : filters.DataFilter, fileNum : int = -1, totalFiles : int = -1) -> commons.AnnotatedData:
    print(f"Converting to LSTM input: {fileNum}/{totalFiles}...")
    session = SessionData.fromPath(eimuPath, fileNum, totalFiles)
    data, timestamps = session.getEimuData(dataFilter=dataFilter)
...
class EimuLSTMInput(commons.ModelType):
    def __init__(self, dataFilter : filters.DataFilter = filters.NoneFilter()):
        self.dataFilter = dataFilter
    
    def fromPath(self, path : str, fileNum : int = -1, totalFiles : int = -1) -> commons.AnnotatedData:
        return eimuToLSTMInput(path, self.dataFilter, fileNum, totalFiles)
